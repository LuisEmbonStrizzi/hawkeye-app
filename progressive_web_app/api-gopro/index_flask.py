...
azure_connection_string = "DefaultEndpointsProtocol=https;AccountName=hawkeyevideos1;AccountKey=SAJ7tOBcdrAPw2vWO0W83u+BNxRrE8yCXGBxwj85QRHpcxgARzfAVf0xhExQaOHCO/OqzZdId/F/+AStpQqQLA==;EndpointSuffix=core.windows.net"
container_name = "pruebavideos"
...
@app.route('/api-gopro/hello', methods=['GET'])
async def hello_world():
    return "Hello, World!"


@app.route('/api-gopro/enable_Wifi', methods=['GET'])
async def enable_wifi():
    try:
        ...
        return jsonify({"networkName": ssid, "password": password})

    except Exception as e:
        logger.error(e)


@app.route('/api-gopro/courtPhoto', methods=['GET'])
async def courtPhoto():
    ...
    # Find the last photo with .jpg extension
        ...
        return jsonify({"message": "PhotoTaken", "file_url": blob_url})

    except Exception as e:  # pylint: disable=broad-exception-caught
        logger.error(e)


@app.route('/api-gopro/record', methods=['GET'])
async def record():
    try:
        async def runCommands():
        ...
        return jsonify({"message": "RecordStarted"})

    except Exception as e:
        logger.error(e)


@app.route('/api-gopro/stopRecording', methods=['GET'])
async def stopRecording():
    ...
    # Find the last photo with .jpg extension
        ...
        # Obtener la URL del blob
        blob_url = blob_client.url
        return jsonify({"message": "CameraConnected", "file_url": blob_url})

    except Exception as e:
        logger.error(e)


@app.route('/api-gopro/getBattery', methods=['GET'])
async def getBattery():
    try:
        ...
        return jsonify({"message": "BatteryHere", "battery": data["status"]["70"]})
