...
def deteccionPorCirculos(preCentro, frame, recorteCerca, correccion, color_pre_centro, resizer):
    ...
    for pixelColorCercano in pixelesColoresCercanos:
        ...
        colores_posible_centro = []

        while contador > 0:
            contador = 0
            for pxl in centro_posible_actual:
                ...
                for i in range(-1, 2):
                    for h in range(-1, 2):
                        if (pxl[0] + i, pxl[1] + h) not in centro_posible_actual:
                            ...
                            #distancia = abs(int(color[0]) - int(color_pre_centro[0])) + abs(int(color[1]) - int(color_pre_centro[1])) + abs(int(color[2]) - int(color_pre_centro[2]))
                            #distancia = abs(int(color[0]) - int(colores[(pxl[0] + i, pxl[1] + h)][0])) + abs(int(color[1]) - int(colores[(pxl[0] + i, pxl[1] + h)][1])) + abs(int(color[2]) - int(colores[(pxl[0] + i, pxl[1] + h)][2]))
                            distancia = abs(int(color[0]) - int(color_a_comparar[0])) + abs(int(color[1]) - int(color_a_comparar[1])) + abs(int(color[2]) - int(color_a_comparar[2]))
                            if distancia <= 20:
                                centro_posible_actual.append((pxl[0] + i, pxl[1] + h))
                                colores_posible_centro.append(color)
                                contador += 1
