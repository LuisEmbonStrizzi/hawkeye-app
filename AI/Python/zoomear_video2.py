...
def zoomear_video(url_video : str, pique : list[(int, int), int]):
    ...
    out = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height)) # Con el códec XVID y ciertos valores tira error
    ...
    while True:
        ...
        if num_frame < start_frame:
            ...
            #frame_zoomed = cv2.copyMakeBorder(frame_zoomed, 0, 1600, 0, 2000, cv2.BORDER_CONSTANT, value=(0, 0, 0))
            cv2.imshow("prueba.jpg", frame_zoomed)
            cv2.imwrite("prueba.jpg", frame_zoomed)
            ...
            print(frame_zoomed.shape)
            print(dy + frame_height, dx + frame_width)
...
if __name__ == '__main__':
    zoomear_video("../InkedInkedTennisBrothersVideo1080p.mp4", [(50, 1080), 1380])