...
def tp_fix(contornos, pre_centro, count):
    ...
    if cnts_pts != []:
        ...
        print("No se encontró la pelota")
...
def main(frame, numeroGlob):
    ...
    frame = imutils.resize(
        frame, anchoOG * resizer_glob[numeroGlob], altoOG * resizer_glob[numeroGlob])
    ...
    if len(cnts) > 0:
        ...
        if len(cnts) > 0:
            if primeraVez_glob[numeroGlob]:
                ...
                M = cv2.moments(c)
                center_glob[numeroGlob] = (
                    int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"])), int(radius)
                # if esResult == False:
                ...
                c = tp_fix(
                    cnts, preCentro_glob[numeroGlob], count_glob[numeroGlob])
            ...
            if radius > 0 and primeraVez_glob[numeroGlob] and c is not None or c is not None:
                ...
                cv2.circle(
                    frame, (center_glob[numeroGlob][0][0], center_glob[numeroGlob][0][1]), 5, (0, 0, 255), -1)
    ...
    if numeroGlob == 0:
        for i in range(1, len(pts_norm)):
            ...
            cv2.line(frame, pts_norm[i - 1][0],
                     pts_norm[i][0], (0, 0, 255), thickness)
        ...
        for i in range(1, len(pts_pers)):
            ...
            # Traza la trayectoria
            thickness = int(np.sqrt(args["buffer"] / float(i + 1)) * 2.5)
            cv2.line(frame, pts_pers[i - 1][0],
                     pts_pers[i][0], (0, 0, 255), thickness)
    ...
    if numeroGlob == 0:
        ...
        if (len(pique2_norm) >= 2):
            ...
            if posiblePique and preCentro_glob[0] is not None and center_glob[0] is not None and (preCentro_glob[0][0][1] > puntoMaximoAbajoCancha * resizer_glob[0] or preCentro_glob[0][0][1] < puntoMaximoArribaCancha * resizer_glob[0] or preCentro_glob[0][0][0] > puntoMaximoDerechaCancha * resizer_glob[0] or preCentro_glob[0][0][0] < puntoMaximoIzquierdaCancha * resizer_glob[0]):
                ...
                print("Center y Mitad de Cancha",
                      preCentro_glob[0], mitadDeCancha)
                if preCentro_glob[0][0][1] <= mitadDeCancha:
                    abajo = False
                else:
                    abajo = True

                if posiblesPiques_pers == []:
                    posiblesPiques_pers.appendleft(
                        (abajo, preCentro_glob[0][0], numeroFrame))
                ...
                if len(posiblesPiques_pers) >= 2:
                    ...
                    if Gerard and type(posiblesPiques_pers[1][0]) is not bool:
                        ...
                        pts_piques_finales.append([posiblesPiques_pers[1][0][0], float(
                            "{:.2f}".format(posiblesPiques_pers[1][1] / fps))])
                        ...
                        pts_golpes_finales.append([posiblesPiques_pers[1][0][0], float(
                            "{:.2f}".format(posiblesPiques_pers[1][1] / fps))])
                ...
                if posiblesPiques_pers == []:
                    posiblesPiques_pers.appendleft(
                        (preCentro_glob[numeroGlob], numeroFrame))
                ...
                countDifPiques = 0
                #frame = cv2.putText(frame, 'Gerard', (preCentro_glob[numeroGlob][0][0], preCentro_glob[numeroGlob][0][1]), cv2.FONT_HERSHEY_COMPLEX, 3, (0, 0, 255), 0, 2)
                pts_pique.append([[preCentro_glob[numeroGlob][0][0], preCentro_glob[numeroGlob][0][1]], float(
                    "{:.2f}".format(numeroFrame / fps))])
    ...
    if velocidad and center_glob[numeroGlob] is not None and numeroGlob == 1 and diferente:
        ...
        velocidadFinal = velocidadGolpe(
            punto1Velocidad, center_glob[numeroGlob], countDifVelocidad)
